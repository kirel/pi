...
- **Management boundary:** The `hermes_baremetal` role manages the runtime envelope: initial bootstrap, read-only update checks, the environment block in `/home/daniel/.hermes/.env`, user systemd services, dashboard metadata, and the host-Caddy publishing integration. Real Hermes updates remain explicit and use `hermes update --backup`. The main `config.yaml`, direct MCP entries, personal provider setup, and 1Password-backed secrets remain manual. Do not replace or broadly template `config.yaml`, remove manual MCP entries, or redeploy the Docker Hermes role when changing this agent.
- **Chat providers:** All Hermes model traffic goes through `custom:kirelabs-litellm`. The cloud default is `gpt-6-astra-cpa` with reasoning `low`; fallbacks are `gemini-3.8-flash-high-cpa`, `glm-5.3-flash` (Coding Plan), then `glm-5.3-flash-openrouter`. Shared defaults live in `group_vars/all/hermes_mcp.yml`; session `/reasoning` overrides remain available. Local Docker Hermes stays on `home-hermes` without automatic fallbacks, with cloud models available through `/model`. Do not restore direct provider keys or OAuth logins in Hermes.
- **LiteLLM use:** LiteLLM provides chat, image generation, `home-asr`, `home-tts`, and the central MCP gateway. The existing LiteLLM key alias is `hermes`; its encrypted value and declarative permissions are managed through `litellm_managed_virtual_keys` in `group_vars/all/hermes_mcp.yml`.
...
- **Service publishing:** The shared Ansible-owned `caddy-proxy` skill uses `hermes-service` for both runtimes. Bare metal publishes below `*.hermes.kirelabs.org` through the Hetzner host Caddy; Docker agents publish below their existing `*.{agent}.kirelabs.org` Caddy sidecars. The Regfish token belongs only to Caddy and must never be exposed to Hermes.
- **Deployment:** Changes to its LiteLLM MCP catalog or managed virtual-key permissions use `uv run ansible-playbook setup.yml --tags llm-tools --limit homelab`. Runtime-envelope changes use `uv run ansible-playbook setup.yml --tags hermes-baremetal --limit ubuntu-8gb-fsn1-1` and require live verification on `100.82.91.51`. Real Hermes updates remain a separate interactive operation.
